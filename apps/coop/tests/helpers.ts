...
export function adminClient(): pg.Client {
  return new Client({
    connectionString: process.env.DATABASE_URL ?? "postgres://postgres:postgres@localhost:5432/bp_coop_test",
  });
}
...
export async function seedCoopChart(client: PoolClient, orgId: string, currency: string): Promise<void> {
  await seedChartOfAccounts(client, orgId, "coop", currency);
}

export async function createTestMember(admin: pg.Client, orgId: string, membershipNumber: string): Promise<string> {
  const result = await admin.query<{ id: string }>(
    `insert into members (org_id, membership_number, full_name, join_date)
     values ($1, $2, $3, current_date)
     returning id`,
    [orgId, membershipNumber, `Member ${membershipNumber}`],
  );
