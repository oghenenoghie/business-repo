...
// Termly's database is core's schema + ledger's schema + this app's own
// (see the root README: "one database per app"). Students/classes/terms
// are org-scoped through core's has_org_role() and fee invoices post
// through ledger's accounts/journal_entries, so a from-scratch database
// needs both applied first.
const migrationSets = [
  { pkg: "core", dir: path.join(appRoot, "..", "..", "packages", "core", "migrations") },
  { pkg: "ledger", dir: path.join(appRoot, "..", "..", "packages", "ledger", "migrations") },
  { pkg: "school", dir: path.join(appRoot, "migrations") },
];

async function main() {
  const databaseUrl = process.env.DATABASE_URL ?? "postgres://postgres:postgres@localhost:5432/bp_school";
  // Defaults to "public" (existing behavior, unchanged for local/CI). Set to
  // e.g. "termly" when deploying into a shared database that already has
