...
// Portier's database is core's schema + ledger's schema + this app's own
// (see the root README: "one database per app"). Properties are org-scoped
// through core's has_org_role() and folio charges post through ledger's
...
const migrationSets = [
  ...
  { pkg: "hotel", dir: path.join(appRoot, "migrations") },
];

async function main() {
  const databaseUrl = process.env.DATABASE_URL ?? "postgres://postgres:postgres@localhost:5432/bp_hotel";
  // Defaults to "public" (existing behavior, unchanged for local/CI). Set to
  // e.g. "portier" when deploying into a shared database that already has
